...
1. **The log message is guarded by its type and a syntactic lint rule only.** `LiteralMessage` is
   ...
   or the leak test stops those. The exported log record's body is also checked against `LOG_MESSAGE_SHAPE` by `scrubbingLogExporter`, which replaces a message with punctuation or structure by `unnamed`, and stops nothing shaped like a word. Never cast into a message. If you need a
