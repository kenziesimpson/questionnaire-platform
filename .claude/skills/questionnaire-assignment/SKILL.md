...
- [[2-design-doc]] — the design doc. Condensed sections, plus the canonical [[2-design-doc#17. Open Questions]]. Italic prompts mark unfilled sections.
- [[12-decisions-log]] — the Decisions Log: one row per decision with alternatives and rationale.
...
- **List endpoints:** every list has an explicit `ORDER BY` on an existing index — `id DESC` (UUIDv7, so the PK index *is* creation order) for `/questions` and `/questionnaires`, `version DESC` for the histories. Not optional: unspecified order is heap order, which shifts after an `UPDATE`. **No pagination** — lists are dozens to hundreds of rows and go to the client whole, so sorting and filtering are client-side; the exit when it is needed is keyset pagination on the same key (Decisions Log #40). That exit is exercised for the first time by the admin responses list ([gh#18](https://github.com/kenziesimpson/questionnaire-platform/issues/18), Decisions Log #88): a questionnaire's session count is unbounded, not dozens-to-hundreds, so it pages by keyset over `(started_at DESC, id)` by default, and by either timestamp in either direction since Decisions Log #90 (in-progress sessions sort last under Submitted) — and each page renders as a plain table with no client-side virtualization, since keyset paging already caps what is ever mounted at once.
...
- **Submit idempotency:** `SHA-256(JSON.stringify(...))` over the validated response rows — sorted by `itemId`, `optionIds` sorted, numbers as exact decimal strings canonicalized by the validator before the row is built (trailing fractional zeros and a trailing point stripped, `-0` → `0`, so `72.50` and `72.5` are one answer), unanswered optional items absent — canonicalized once in `@qp/shared` (Decisions Log #37). Digest the rows, never the request body. It must stay a pure function of the persisted `response` rows, which is what makes a later change a backfill.
...
- **API boundary:** the boundary is an **artifact, not a route prefix**. Exactly one object crosses it — the immutable `PublishedDefinition` snapshot — and the dependency runs one way; execution never reads a draft, resolves a `questionId`, or joins to an authoring table. Enforced in three layers: encapsulated Fastify plugins with no cross-imports (ESLint zone rule), wire types in `@qp/shared`, and two database roles for these two halves specifically (`qp_execution` has no grant on authoring tables; `qp_definition` has **no grant on `response`**). `/api/definition/*` owns bank, drafts, publish, retire and version history; `/api/run/*` is three routes and has **no unpinned definition read**, so mid-session version drift is unrepresentable. RFC 9457 `problem+json`, `400` schema-only / `409` state conflict / `422` domain-invalid, and error bodies never echo a submitted answer. Submit is server-authoritative, all-or-nothing, idempotent on the session via an answer digest. Two plugins in one process now for this boundary; splitting into two services is a deployment change, kept cheap deliberately. Detail in [[7-application-boundary]].
...
- **A third database role, `qp_reporting`, backs the admin responses browser** ([gh#18](https://github.com/kenziesimpson/questionnaire-platform/issues/18), design doc Decisions Log #89) — a third encapsulated Fastify plugin (`modules/reporting`, `/api/reporting`) alongside definition and execution, so the backend now runs three plugins and three pools, not two. `qp_reporting` holds `SELECT` on `execution.session`, `execution.response`, the `definition.published_questionnaire_version` view and the `id` column of `definition.questionnaire`, and `EXECUTE` on `audit.record` (migration `0020`, limited to the `view_response` action by `0022`) for the audit row a detail read writes, and nothing else; nothing qp_definition or qp_execution already hold changes; a deliberately narrow slice of the "third read-only surface" [[2-design-doc#17. Open Questions]] §3 anticipated, not that whole surface (no domain events, no audited export, no aggregation). `modules/reporting` holds only that role's pool and never borrows `qp_execution`'s, so the module cannot write except through `audit.record`, by grant, not by convention (the borrowed-pool design was reversed in review). It is unauthenticated like every other route, and its list enumerates in-progress session ids, each a bearer capability for the respondent routes — [[2-design-doc#17. Open Questions]] §3.
...
- **Admin:** seven screens — questionnaire list, draft editor, question bank, version history, preview, and (Wave 3a) a two-screen unaggregated responses browser ([gh#18](https://github.com/kenziesimpson/questionnaire-platform/issues/18)): a paginated, filterable session list and a session detail screen against that session's own pinned version. The bank has its own screen *and* questions can be created/edited inside the draft editor, where **editing re-pins that item** to the new version. Predicate editor is a flat row list offering only earlier questions and only type-valid operators. Draft writes go through the `If-Match` ETag with optimistic reorder and rollback on `409`. The responses browser is explicitly *not* the admin reporting surface ([[2-design-doc#17. Open Questions]] §3, [[7-application-boundary#10. Open questions]] §2), which stays undesigned; it does not aggregate, filter server-side beyond version/status, or export (its one sort control is the Started/Submitted column headers, Decisions Log #90), and who may read it is still open — see gh#18 and Decisions Log #89 for what shipped and what stayed open.
...
Anything deferred rather than blocking lives in [[2-design-doc#17. Open Questions]] — check there before treating something as undecided. Deliberate simplifications in the format and rule model are listed in [[5-questionnaire-format#8. Future changes noted]]; they are settled, not open.
...
- The design doc and its companions always own two things regardless of how many detail docs exist: the [[12-decisions-log]] table (one row per decision — a named deliverable in the brief) and [[2-design-doc#17. Open Questions]] (everything punted, deferred or incomplete, in one place).
...
3. Add a [[12-decisions-log]] row: decision, alternatives considered, rationale, date.
4. Put the expanded alternatives in the detail doc's own "Alternatives considered" section. [[12-decisions-log#Alternatives considered in detail]] keeps only decisions that have no detail doc, and points at the rest.
5. Move anything newly deferred into [[2-design-doc#17. Open Questions]], and tick [[4-implementation-plan]].
