...
- **A third database role, `qp_reporting`, backs the admin responses browser** ([gh#18](https://github.com/kenziesimpson/questionnaire-platform/issues/18), design doc Decisions Log #88) — a third encapsulated Fastify plugin (`modules/reporting`, `/api/reporting`) alongside definition and execution, so the backend now runs three plugins and three pools, not two. `qp_reporting` is granted exactly `SELECT` on `execution.session` and `execution.response`, nothing on `definition.*` and nothing qp_definition or qp_execution already hold changes — a deliberately narrow slice of the "third read-only surface" [[2-design-doc#18. Open Questions]] §8 anticipated, not that whole surface (no domain events, no audited export, no aggregation). It resolves a session's pinned snapshot by reusing `qp_execution`'s own existing grant on `definition.published_questionnaire_version`, read-only, rather than taking a fresh grant of its own.
