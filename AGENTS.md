...
- Icons come only from `lucide-react`, re-exported through `@qp/ui/icons` (`packages/ui/src/icons.ts`). Only that file and `packages/ui/src/primitives/**` import `lucide-react` directly, and no inline `<svg>` appears outside `packages/ui`. ESLint rejects the direct import; the inline `<svg>` is a warning.
- Respondent answer values never reach a log, span, metric or error body. Wrap them in `Sensitive<T>`.
- A value two modules must agree on is declared once, in the narrowest module that owns it, and union types are derived from an `as const` array rather than hand-written beside one. `.claude/skills/constants/SKILL.md` has the rule, the cases where duplication is deliberate, and where a shared module is warranted.
