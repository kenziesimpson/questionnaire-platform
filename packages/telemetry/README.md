...
| Export | What it does |
| --- | --- |
| `createEventQueue({ send, beacon, screen?, maxPending?, batchSize?, flushIntervalMs? })` | A bounded queue of `{ level, message, attributes }` events. `enqueue` scrubs before it queues and never throws; `flush()` sends through `send`; `flushOnExit()` hands everything left to `beacon`; `stats()` reports pending, sent and every drop |
...
| `installErrorCapture(queue, window)`, `captureError(queue, kind, error)` | An `error` and an `unhandledrejection` listener, and the same capture for an error boundary. Records the error's class name and its stack frames only |
| `startBrowserTracing()`, `stopBrowserTracing()`, `injectTraceHeaders(headers)` | A web tracer provider with a synchronous context manager, and the `traceparent` of the active span added to a headers record. No `instrumentation-fetch` and no patching of global `fetch` (O12): the app's one `fetch` wrapper calls `injectTraceHeaders` by hand |
...
| `startBrowserTelemetry({ page, ...queue options, debug? })` | Tracing, the queue, log routing, page-hide flush and error capture in one call; `stop()` undoes them |
...
- **An error is its class name and its frames, never its message.** `stackFramesOf` still decides whether the stack lines up with the message and keeps only frame-shaped lines; the browser then cuts each frame's location to its script file name, `index-3f9a.js:10:20`, or `anonymous.js` for anything else, so no page URL or session id in a path survives. Stack frames in another browser's format (`fn@url:1:2`) do not line up, so those errors carry a type and no frames. The capture never reads an error event's `message`, `filename` or position.
...
| `src/browser.ts`, `src/browser/` | The browser entry point and its parts: `queue.ts`, `events.ts` (the queued event and its scrub), `errors.ts`, `lifecycle.ts`, `logging.ts`, `idle.ts`, `tracing.ts`, `start.ts`, `page.ts` (the structural types for `window`) |
