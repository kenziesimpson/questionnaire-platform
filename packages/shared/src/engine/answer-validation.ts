...
import { type QuestionContent, type QuestionOf, freeformOptionOf, optionIdsOf } from "../domain/question.js";
...
function choiceCodes(
  question: QuestionOf<"single_choice" | "multiple_choice">,
  ...
  const freeformOtherSelected = other !== undefined && optionIds.includes(other.optionId);
...
function multipleChoiceCodes(
  ...
  const codes = choiceCodes(question, answer.optionIds, answer.otherText);
  ...
  switch (question.type) {
    ...
    case "single_choice":
      return answer.type === "single_choice"
        ? choiceCodes(question, [answer.optionId], answer.otherText)
