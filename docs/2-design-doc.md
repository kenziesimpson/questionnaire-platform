...
- **Published rows reject `UPDATE` and `DELETE`.** §6.4 specifies the `UPDATE` trigger; `DELETE` needs
  the same guard for the same reason, and draft items are guarded against their parent's status with a
  locking read, because the naive version of that trigger races a concurrent publish and lets the
  snapshot and the item rows disagree.
...
- **Collected responses are immutable and the audit log is append-only, both by grant.**
  `qp_execution` has `SELECT, INSERT` on `response` and nothing else; no role has `DELETE` anywhere.
...
**Decision:** OpenTelemetry end to end, with respondent answer values structurally excluded from every signal. Full reasoning, field lists, metric names and enforcement design in [[6-observability]]. See Decisions Log #16.
...
- **One command locally; two jobs in CI.** A root Vitest config with `projects` makes `npm test` run all three Vitest suites; `npm run test:e2e` stays separate. CI splits checks (lint, typecheck, `vitest run`, the telemetry canary gate) from the end-to-end job that builds images and brings up compose, so a typo does not wait on a Docker build. Because the specs need only a `baseURL`, the same three run after a deployment as a smoke test — which is the answer to how this works under Kubernetes without a second suite.
...
[[5-questionnaire-format#7. Alternatives considered]] rather than repeated here. Decisions #17–#21 concern
...
6. **Checkpoint endpoint** — ship `PUT /sessions/:id/progress` in the prototype or defer. The only route in the execution API that is not load-bearing: partial answers already live in the browser, so its value is operational (seeing *where* a session stopped, not merely that it did) plus cross-device resume, against a write path on the hot side and a partial-answer store with its own redaction surface. Also in [[3-scaling#8. Open questions]].
...
12. **Discarding never-published drafts.** The one plausible exception to "nothing is deleted" ([[#3. Constraints]]): a draft that was never published is work in progress rather than a record, and an author who abandons one has no way to clear it. Punted for now — drafts are harmless at prototype scale and the partial unique index already limits them to one per questionnaire. The decision is not free later: it determines whether `questionnaire_item` carries `ON DELETE CASCADE` and whether `qp_definition` is granted `DELETE` at all ([[#12. Database]]), so the grants ship without `DELETE` until this is settled. Question versions are out of scope here — those are append-only by [[2-design-doc#17. Decisions Log]] #13 regardless.
