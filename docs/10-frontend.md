...
**Editing a question from within the draft editor re-pins that item to the new version.** The gesture is unambiguous — the author is editing this question in the context of this draft — and other questionnaires' items are untouched, because each pins its own version. This is the deferred "upgrade to latest" capability ([[2-design-doc#18. Future Work]]) scoped to one item and triggered by an edit, rather than offered as a general review action.
...
1. **Single-page rendering past demo size.** The model is fine for a handful of items and degrades for a long questionnaire. Pages and sections are already deferred ([[2-design-doc#18. Future Work]]); if they arrive, the rendering model is the thing they change.
