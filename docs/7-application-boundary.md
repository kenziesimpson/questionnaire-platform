...
5. **A taken `key` on create — open, deferred 2026-09-13.** `question.key` and `questionnaire.key` are unique, and `POST /questions` or `POST /questionnaires` with a key already in use currently fails with `500 internal`: the definition error handler maps only `question_version_pkey`'s `23505` and deliberately leaves every other unique violation unhandled, and `_tests/modules/definition/errors.test.ts` asserts that. No slug in §6.1 fits — it is not a version conflict, not draft content, and `400` is reserved for client bugs — so the likely shape is a new `409` slug plus an entry in the closed union in `@qp/shared`, but the choice was deliberately not made during Track 4. **Must be resolved before the admin app ships key entry (Track 6)**, where it becomes a reachable user mistake rather than a theoretical one.
