...
3. **Numbers enter as their canonical exact decimal string.** `number_value` is `numeric` so an evidentiary record does not round ([[9-database-schema#Responses]]); putting a JS number through `JSON.stringify` would undo that in the digest. The validator canonicalizes the wire string **before the row is built** — trailing fractional zeros and a trailing point are stripped, and `-0` becomes `0` — so `72.5`, `72.50` and `72.500` are one answer with one row and one digest, and an `integer` question accepts `72.0` as `72`. It happens at the row rather than inside the digest because `numeric` preserves the input's scale: a digest-only canonicalization would disagree with the digest recomputed from the stored column, breaking the pure-function constraint below. Range checks compare the exact value and are unaffected.
...
- **Two concurrent saves of one question** are serialized by `SELECT ... FOR UPDATE` on the question row before the next version number is computed ([[9-database-schema#Row locks]]), so both succeed as *N+1* and *N+2*. `question/version-conflict` maps the `23505` that the lock is supposed to make unreachable — a safety net that should never fire, not the normal path. This does not reopen Decisions Log #31: concurrent edits stay unguarded against *lost updates*, which is a different question from the primary-key race.
...
Against that: a write on the hot side of the system, and a partial-answer store that becomes a second place raw answers live, with its own grant, its own redaction surface and its own retention question. The shape is recorded in [[9-database-schema#9. Not built]] so that adding it later is additive — one table, one grant, one route, and nothing existing to migrate.

## 10. Open questions

1. **Checkpoint endpoint — resolved: deferred** (Decisions Log #25). `PUT /sessions/:id/progress` does not ship in the prototype. Without auth the session id lives and dies in the same browser storage as the answers it would recover, so the cross-device-resume benefit is largely unavailable, and the operational half is already carried by domain events. Reasoning in §9.7; the table shape is recorded in [[9-database-schema#9. Not built]] so adding it stays additive.
