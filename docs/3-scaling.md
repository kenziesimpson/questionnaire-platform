...
- **No checkpoint endpoint.** A debounced `PUT /sessions/:id/progress` upserting answers-so-far was considered and deferred ([[12-decisions-log]] #25): without auth the session id sits in the same browser storage as the answers, so the server-side copy is unreachable in the cases that would need it. Partial answers stay client-side and nothing is written to the server between session start and submit.
...
2. **Connection pooling.** PgBouncer (or managed pooler) in transaction mode in front of the primary ([[12-decisions-log]] #78). The trigger is not "replicas multiply" but the fourth backend *process*: three pools (`qp_definition`, `qp_execution`, `qp_reporting`) at `pg`'s default of ten make 30 per process, and a stock instance leaves 97, so three replicas at 25% rollout surge (which rounds up to a fourth pod) — or one 8-core box under [gh#19](https://github.com/kenziesimpson/questionnaire-platform/issues/19)'s per-core cluster workers — reaches it. Beneath it, per-role `CONNECTION LIMIT` keeps execution from starving definition or locking `qp_owner` out of migrations. Transaction mode discards session state, which this design survives because the roles connect as distinct users rather than `SET ROLE` on a shared connection. That is also what keeps `audit.record`'s `session_user` guard for `qp_reporting` sound ([[9-database-schema#9.1 A dedicated role, inside the publish transaction]]): the pooler must not connect as a fixed server user, or every caller looks the same.
...
- **Not mitigated in the prototype.** The checkpoint endpoint is deferred ([[12-decisions-log]] #25), and the honest reason is that it would not have helped much: the session id is lost in the same events that lose the answers, so a server-side copy has nothing to address it with until there is an identity to look a session up by. That identity is the real prerequisite for full resume.

## 8. Open questions
- ~~Ship the checkpoint endpoint in the prototype or defer?~~ **Resolved: deferred** ([[12-decisions-log]] #25).
