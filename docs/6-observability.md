...
| Error message, in the Postgres log | The same primary message is written to the server log, and `terse` does not remove it | Not removed by configuration, and V1 shows it is written: a bound value whose cast fails is in the `ERROR:` line, and only there (`postgres-log.test.ts`). What keeps a value out is request validation, which runs before a value is bound. V1 sent 26 malformed values through the real app, in a session id, a questionnaire id, a question id, a cursor (not a cursor, built from the value, and shaped like one with a bad session id), a `version`, `status`, `sort` and `order`, an unknown query parameter, an `If-Match` header, a close time, a key and the answers of a submit (a date, an option id, a decimal and an item key), and found none in the log (`postgres-log-routes.test.ts`). **Two values validation admits and Postgres rejects are the exception, and are open findings.** A positive integer above 2147483647 passes the schema for a published version, a question version and the responses list's `version` filter and fails as `value "<n>" is out of range for type integer`; a date answer whose year is `0000` passes the schema and every question rule and fails as `date/time field value out of range: "<date>"`. The first is a number a caller typed into a URL; the second is a date a respondent typed as an answer. The schemas' acceptance of both values was checked directly and the messages are Postgres's own; the log line is observed by the two tests, which assert the message is in the log, so they fail when validation is tightened (a maximum on `PositiveInt`, a year of at least 1 on `IsoDate`), and are then changed to assert it is absent. A draft item's `questionVersion` is the same schema type and was not exercised | V1 |
