...
| Fewer than one month of future `response` partitions remain | `monitor.response_partition_months_ahead()`, read as `qp_monitor`; the alert is the value `0` (§14, [[9-database-schema#10.1 `qp_monitor` and the `monitor` schema]]) | D2 |
...
| A role reading the stats | `pg_read_all_stats` shows every role's statement text and counters, and the extension grants its views to `PUBLIC`, where each role sees its own text | `qp_monitor` is the only role granted `pg_read_all_stats`, through `pg_monitor`, and the roles script revokes the two views from `PUBLIC` and grants them to `qp_monitor` alone. The only text it can see is normalised. It holds no grant on an answer table | D2 |
