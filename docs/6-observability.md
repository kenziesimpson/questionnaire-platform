...
`qp_reporting` also holds `EXECUTE` on that function (O14), and is limited to one action: `audit.record` refuses a call whose `session_user` is `qp_reporting` unless the action is `view_response` (migration `0022`, A1; [[9-database-schema#9.1 A dedicated role, inside the publish transaction]]). The repository function `recordResponseView` names only that action too, so the limit holds in code and in the database independently. Nothing on this path is telemetry: the row's `summary` is the session id and nothing else.
