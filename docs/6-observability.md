...
| Caller-supplied trace headers in the SQL comment | An inbound `tracestate` (or any propagated header) copied into the statement's comment, and so into `pg_stat_activity` and the Postgres log | Shipped: the registered propagator handles `traceparent` only; tested on the propagator, on a fake driver and on the real client | D1 |
