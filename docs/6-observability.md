...
- **Tail sampling in the Collector (P1, shipped): every error trace, every slow trace and 10% of the rest.** The decision is made after the whole trace has arrived, which is the only way to keep a trace *because* it failed or was slow. A head sampler cannot do this: it decides at the first span, before the outcome exists. The `tail_sampling` processor holds a trace for `decision_wait: 10s` and keeps it when any of three policies say so:
  ...
  - `health_probes`: a `drop` policy for any trace with a span on `/health/live` or `/health/ready`, which overrides the other three. The healthcheck calls `/health/ready` every five seconds and each call runs a query on three pools, so without it the baseline would keep about one probe trace every fifty seconds. A failing readiness check still shows in the `readiness check failed` warning log and in the container's health.
...
- The redaction processor removes attributes, including resource and scope attributes, and nothing else. A span's name, a span event's name, a status message, an exemplar, a metric's name and a log record's body pass through it unchanged, so they rest on the SDK's scrub (§3.1) and, for a log body, on the Collector's message filter (§11). The residual case is a client that is not our SDK: anything on the Compose network can post to the Collector's OTLP port, and nothing but the SDK's own scrub stops such a client putting a value in a name. The port is published on `127.0.0.1` only.
