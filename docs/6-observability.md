...
| A role reading the stats | `pg_read_all_stats` shows every role's statement text and counters | `qp_monitor` is the only role granted it, through `pg_monitor`, and the only text it can see is normalised. It holds no grant on an answer table | D2 |
