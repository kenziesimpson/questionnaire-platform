...
| Case | File | Invariant defended | §3 row |
| --- | --- | --- | --- |
| Every operator of every response type — 22 operators, each with a satisfying and a non-satisfying answer, including `between` at both inclusive ends; a guard asserts the table covers the full operator set | `engine/visibility.test.ts` | One evaluator, typed per response type ([[5-questionnaire-format#4.3 Evaluation]], #9) | Conditional navigation over one and several earlier answers |
| Number conditions compare the decimal string exactly against the authored constant: `18.00` equals `18`, `0.1` is not `lt 0.1`, `1e21` and `1e-7` operands compare beyond double precision | `engine/visibility.test.ts` | #42: exactness survives into evaluation, not only storage | Conditional navigation over one and several earlier answers |
| Unanswered → `false` for every operator except `notAnswered`; the negative operators `isNot`, `isNoneOf`, `excludes`, `neq` do not fire for a respondent who never answered | `engine/visibility.test.ts` | A condition means the answer exists and satisfies the operator (§4.3) | Absent-answer semantics |
| Referenced item hidden with a stale answer kept → `false` for every operator, `notAnswered` included; `notAnswered` is `true` only while the referenced item is shown and unanswered | `engine/visibility.test.ts` | A condition on a question that was not shown is `false` (§4.3); answers to hidden items are kept client-side ([[10-frontend#4.3 Local persistence]]) | Absent-answer semantics |
| An answer shaped for another type, an explicit `null`, an itemId named like an `Object.prototype` key, and a reference to a later item are all treated as not answered or not shown | `engine/visibility.test.ts` | Absent-answer semantics hold for malformed and adversarial input | Absent-answer semantics |
...
| Exact decimal comparison (trailing zeros, `-0`, beyond 2^53), authored constants read as their shortest decimal including exponent forms, integer detection, negative-zero normalization | `engine/decimal.test.ts` | #42 exactness in comparison and the stored row | Response validation: required, per-type value rules |
| Canonical form is exactly §5.4's: sorted by `itemId`, `optionIds` sorted, decimal strings verbatim, absent fields omitted, fixed key order | `engine/digest.test.ts` | #37 canonicalization, specified once in `@qp/shared` | Submit idempotency: same digest replays, different digest conflicts |
...
| A JSON round-trip of the stored columns digests identically; `-0` recomputed from the stored `0` matches; null and omitted optional answers and a reordered request body digest identically through `validateSubmission` | `engine/digest.test.ts` | The digest stays a pure function of the persisted rows (#37) | Submit idempotency: same digest replays, different digest conflicts |
| Changing an option, `otherText`, date, text, the decimal string, the unit, or adding / removing an answer changes the digest | `engine/digest.test.ts` | Canonicalization loses no information, so no false `200` | Submit idempotency: same digest replays, different digest conflicts |
