...
| Starting is `201` while `closes_at` is in the future and `409 questionnaire/closed` at and after the instant, under an injected clock | `_tests/modules/execution/sessions.test.ts` | One `closes_at` comparison is the retirement lifecycle ([[2-design-doc#8.1 Questionnaire lifecycle and retirement]]) | Retirement: start and submit rejected past `closes_at` |
...
| The module's database handle is `qp_execution` and is `42501` on `question`, `question_version`, `question_version_option`, `questionnaire_item` and the base `questionnaire_version` | `_tests/modules/execution/boundary.test.ts` | Execution cannot reach authoring tables ([[7-application-boundary#3.2 Database grants]]) | The definition/execution barrier |
...
| `buildApp` mounts the module at `/api/run`, and unknown routes inside and outside it are `404` problem details | `_tests/modules/execution/boundary.test.ts` | One error format on every non-2xx (§6.1) | — contract |
