...
| Case | File | Invariant defended | §3 row |
| --- | --- | --- | --- |
| `UPDATE` of a published snapshot, from `qp_definition` and from `qp_owner`, is rejected `QP001` — a direct client bypassing the API | `_tests/db/immutability.test.ts` | Layer 1 and layer 3 of [[5-questionnaire-format#6.4 Three layers of immutability enforcement]] | Questionnaire versioning: publish, immutability, one draft |
...
| Items of a published version reject `INSERT`, `UPDATE`, `DELETE` and reparenting into a draft; draft items stay editable | `_tests/db/immutability.test.ts` | The item guard and its reparent check ([[9-database-schema#4.1 The item guard, and the two ways it fails naively]]) | Questionnaire versioning: publish, immutability, one draft |
...
| `qp_execution` cannot read `question`, `question_version`, `question_version_option`, `questionnaire_item`, cannot write any definition table, and can read questionnaires, published versions and `version_question_index` | `_tests/db/grants.test.ts` | [[7-application-boundary#3.2 Database grants]] | The definition/execution barrier |
| `qp_execution` cannot `UPDATE` or `DELETE` a response or `DELETE` a session; neither application role holds `DELETE` or `TRUNCATE` on any table | `_tests/db/grants.test.ts` | Collected responses are immutable by grant (#24) | The definition/execution barrier |
...
| A stale draft revision, and failing publish rules, return an outcome and write nothing, audit included | `_tests/db/definition/publish.test.ts` | #43 revision check; a failed publish leaves no audit record | Questionnaire versioning: publish, immutability, one draft |
...
| The seed publishes exactly the documented v1 snapshot under the hardcoded ids, `qst_which_condition` at question version 3, through the audited authoring path; a second run is a no-op; a rejected publish rolls the whole seed back | `_tests/db/demo-questionnaire.test.ts` | #35 hardcoded ids; [[9-database-schema#11.6 The seed is an integration test wearing a disguise]] | Response meaning preserved across a republish |
