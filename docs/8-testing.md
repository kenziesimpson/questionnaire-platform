...
| Case | File | Invariant defended | §3 row |
| --- | --- | --- | --- |
| `POST /questions` is `201` with a `Question` at version 1, its options in authored order, `createdBy` `prototype-author`, and one `create_question_version` audit row whose actor is `prototype-author` | `_tests/modules/definition/routes/questions.test.ts` | Creating a question writes version 1 (#13); every authoring write records the placeholder author, never `null` (#53) | Question versioning: append-only, items pin a version at add time |
...
| `POST /questions/:questionId/archive` is `200` with `archivedAt` set; archiving again is `200` with the same body and writes no second `archive_question` audit row; the one row's actor is `prototype-author` | `_tests/modules/definition/routes/questions.test.ts` | `archived_at` is set once, and the audit trail records the archive once (#15, #53) | Question versioning: append-only, items pin a version at add time |
...
| Case | File | Invariant defended | §3 row |
| --- | --- | --- | --- |
| `POST /questionnaires` is `201` with a `QuestionnaireSummary` (`currentVersion` and `closesAt` null, `hasDraft` true); its draft reads back with no items, `ETag` at revision `0` and `Cache-Control: no-store` | `_tests/modules/definition/routes/drafts.test.ts` | Creating a questionnaire opens draft version 1 ([[7-application-boundary#4.1 Endpoints]]); the draft ETag is built from the revision counter (#43) | Questionnaire versioning: publish, immutability, one draft |
| Create, save and open-next-draft record `prototype-author` as the audit `actor_id` and as `questionnaire_version.created_by` | `_tests/modules/definition/routes/drafts.test.ts` | #53: every authoring write stores the placeholder through `authorOf`, never `null` | — access model |
...
| `lockQuestionnaire` returns the locked row's id and `closesAt`, and nothing for an unknown questionnaire | `_tests/db/definition/questionnaire-rows.test.ts` | One questionnaire lock shared by publish, open-next-draft and retire, returning what each needs ([[9-database-schema#5. Concurrency control]]) | Questionnaire versioning: publish, immutability, one draft |
| While one transaction holds `lockQuestionnaire`, a second transaction's `lockQuestionnaire` on the same row is seen waiting on a lock in `pg_stat_activity` and acquires it only after the first commits | `_tests/db/definition/questionnaire-rows.test.ts` | [[9-database-schema#5. Concurrency control]]: the questionnaire `FOR UPDATE` serializes publish against create-next-draft, the race that silently reverts a version | Questionnaire versioning: publish, immutability, one draft |
...
| `readOpenDraft` finds nothing when only a published version exists, then returns the next draft's id, title, `updatedAt` and revision, the same row locked or unlocked | `_tests/db/definition/questionnaire-rows.test.ts` | "Open draft" is defined once as `status = 'draft'` for the questionnaire; a published version is never read as the draft (#8) | Questionnaire versioning: publish, immutability, one draft |
| While one transaction holds the draft row through `readOpenDraft` `for-update`, an `unlocked` read returns at once and a `for-update` read waits until the first commits | `_tests/db/definition/questionnaire-rows.test.ts` | The lock-or-not choice is explicit and honoured, so publish and replace keep their draft row lock and plain reads never block | Questionnaire versioning: publish, immutability, one draft |
| `hasOpenDraft` and the correlated `openDraftExists` agree for a draft-only, a published-only and a republished-with-draft questionnaire, and `hasOpenDraft` is false for an unknown id | `_tests/db/definition/questionnaire-rows.test.ts` | `hasDraft` on the list and on `setClosesAt` come from one condition, so they cannot disagree | Questionnaire versioning: publish, immutability, one draft |
...
| `PUT /closes-at` sets (normalizing an offset to UTC), reschedules and clears `closesAt`, each `200 QuestionnaireSummary`; audited `retire`, `retire`, `reopen` with `{ from, to }` and `prototype-author` as actor | `_tests/modules/definition/routes/versions.test.ts` | Retirement is one nullable `closes_at`, and clearing it undoes a premature retirement ([[2-design-doc#8.1 Questionnaire lifecycle and retirement]], [[7-application-boundary#4.3 Publish and retire]]) | Retirement: start and submit rejected past `closes_at` — the write that sets it |
| `PUT /closes-at` on a never-published questionnaire reports `currentVersion: null` and `hasDraft: true` | `_tests/modules/definition/routes/versions.test.ts` | The response is the same `QuestionnaireSummary` as `GET /questionnaires` | Retirement: start and submit rejected past `closes_at` — the write that sets it |
| `PUT /closes-at` on an unknown questionnaire is `404` and writes no audit row | `_tests/modules/definition/routes/versions.test.ts` | The audit row commits only with the domain change ([[9-database-schema#9.1 A dedicated role, inside the publish transaction]]) | Retirement: start and submit rejected past `closes_at` — the write that sets it |
