...
| `conditionHolds`: unanswered → `false` for every operator except `notAnswered`; not shown with an answer kept → `false` for every operator; an answer shaped for another type → `false`; the negative operators `isNot`, `isNoneOf`, `excludes`, `neq` do not fire for a respondent who never answered; `notAnswered` holds only while shown and unanswered | `engine/conditions.test.ts` | A condition means the answer exists and satisfies the operator (§4.3) | Absent-answer semantics |
...
| Traversal: every operator-table case evaluated through a gated definition gives the same result as `conditionHolds`; with the gate closed and the stale answer kept, every operator is `false`; `notAnswered` follows the gate | `engine/visibility.test.ts` | A condition on a question that was not shown is `false` (§4.3); answers to hidden items are kept client-side ([[10-frontend#4.3 Local persistence]]) | Absent-answer semantics |
