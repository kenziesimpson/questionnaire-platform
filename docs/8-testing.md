...
| A selected freeform `other` with missing, empty or whitespace-only `otherText` → `choice/other-text-required`, for single and multiple choice; padded text passes; whitespace-only text without `other` stays `choice/other-text-without-other` | `engine/answer-validation.test.ts` | An `other` answer carries its text; the stored row never holds a blank freeform answer | Response validation: required, per-type value rules |
