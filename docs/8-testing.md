...
| The catalogue has a message for exactly the submission codes, minus `answer/not-visible` and `answer/unknown-item`, plus `choice/other-text-required`; messages read bounds from the question and fall back when a bound is absent | `questionnaire/messages.test.tsx` | One code-to-message catalogue for client and server errors | Response validation: required, per-type value rules |
