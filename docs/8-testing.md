...
| --- | --- | --- | --- |
| Every closed literal union in the contract is built with `LiteralUnion`, and its static type is exactly its value list rather than `never`: `ResponseType`, `QuestionVersionSummary.type`, the `validateDraft` item `code`, the `ProblemDetails` item `code` and `ProblemDetails.type`, compile-time checked; at runtime each admits exactly its values and emits one `const` per value | `primitives.test.ts` | A union mapped from an array is `never` to TypeBox's tuple-recursive `Static`, so a handler returning one of these shapes could not compile without a cast; the wire schema is unchanged | — contract |
