...
| FE1 | Frontend | Respondent: trace headers through `injectTraceHeaders` (accepting `Headers` and array inputs), abandonment beacon, page-speed metrics, errors. Tracing is opt-in so the respondent bundle ships `sdk-trace-web` only when used, measured on the bundle; a type-level test that a DOM `Window` is assignable to `PageWindow` | FE0b | Merged (#150) |
...
- [ ] **H7** *(after Track 8)* Enable the opt-in profile, submit once, follow the backend trace of that submit from the browser's `traceparent` to its database statements and its joined logs, and confirm no answer value appears in any trace, log or metric. Then open the response in the admin app and confirm the detail read wrote exactly one `view_response` audit row, that a window refocus wrote no second one, and that no answer value appears in the read's trace or logs. Browser spans are never exported (there is no browser span exporter, [[6-observability#6. Client-side telemetry]]), so there is no client span to follow in Tempo; the client side of the trace is the parent id of the backend `request` span, which is the span id in the request's `traceparent` and in nginx's access-log line.

  **Set up.** This is the observability profile with a frontend built to trace, and neither is the default.
  1. `cp .env.example .env`, and in `.env` set `OTEL_EXPORTER_OTLP_ENDPOINT=http://collector:4318`.
  2. Build the frontend image with tracing on. Compose does not pass the build argument, so name it: `docker compose -f docker-compose.yml build --build-arg VITE_TELEMETRY_TRACING=true frontend`. (Without it the browser sends no `traceparent`, the backend trace has no browser parent, and step 3 finds nothing.)
  3. Start it: `docker compose -f docker-compose.yml --profile observability up` (add no `--build`, which would rebuild the frontend without the argument). Wait for the `lgtm` container to accept traffic, tens of seconds; the first exports after a cold start may be dropped ([[6-observability#11. Local and hosted setup]]).
  4. Open the respondent at http://localhost:8080 and fill in the intake questionnaire, typing a marker you can search for, such as `h7marker4172`, into the pharmacy answer and into "Other" on the condition question. Submit. Open Grafana at http://localhost:3001 (no login).

  **Follow the trace** ([[6-observability#11.3 Following one request in Grafana]]).
  1. Explore, the Tempo data source, TraceQL: `{ name = "request" && span:parentID != nil }`. Open the trace for `POST /api/run/sessions/:sessionId/submit`. Its root is shown as `<root span not yet received>`: the browser's `browser.request` span is its parent and is never exported. Under the backend `request` span are the handler, `session.submit`, `rule.evaluate` and the `pg.query:` spans of the submit's transaction. Note the trace id.
  2. Find the browser side: `docker compose -f docker-compose.yml logs frontend` shows the nginx line for that request with the same `trace_id` and a `span_id`. Search Tempo for `{ span:parentID = "<that span_id>" }` and find the same `request` span. (Read the nginx log with `docker compose logs`; `tail` on the file inside the container blocks.)
  3. Explore, the Loki data source: `{service_name="qp-backend"} | trace_id="<trace id>"`. Expect the request lines and the submit's events, `session.question_answered`, `session.completed` and `session.submit_finished`, each with the trace id of step 1.
  ...
  **Trace and log join, in short.** For any backend request: (a) the Tempo trace id, (b) `{service_name="qp-backend"} | trace_id="<id>"` in Loki returns its log lines and each line's span id is one of the trace's spans, (c) nginx's line for the request has the same trace id, and (d) a statement in `pg_stat_activity`, read within ten seconds of the request because the pool closes idle connections after that (`docker compose -f docker-compose.yml exec db psql -U questionnaire -d questionnaire_platform -c "SELECT application_name, left(query, 160) FROM pg_stat_activity WHERE application_name LIKE 'qp-backend:%'"`), ends in `/*traceparent='00-<trace id>-<span id>-01'*/`, whose span id is a `pg.query:` span of the trace. The same join runs without Grafana in `apps/backend/_tests/trace-continuity.test.ts`.
