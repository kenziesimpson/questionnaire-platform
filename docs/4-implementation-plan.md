...
4. **The author is `authorOf(request)`**, passed as `actorId` / `createdBy` on every write. Never `null`, and never the placeholder string typed at a call site. See [[2-design-doc#17. Decisions Log]] #57 before writing anything that stores it. `traceId` is `auditTraceId()` from `src/http/trace.ts`: the trace id of the active span, whether or not it is sampled, or `null` when no span is active.
