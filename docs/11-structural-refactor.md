...
| L5 | No prose comments | Rejects any comment except ESLint and TypeScript directives (`eslint-*`, `@ts-expect-error — <reason>`). **Amended:** scoped to `packages/shared/**` and `packages/telemetry/**`, the directories PR 18 actually cleared, rather than every file — three prose comments survive outside that scope (`apps/backend/src/db/definition/versions.ts`, owned by PR 4; `eslint.config.mjs`'s own header, PR 0's; `vitest.config.mts`, unowned by any PR in this pass) and a repo-wide rule would fail lint on files this PR does not touch | A local rule in `eslint.config.mjs` | 18 | about 190 lines |
