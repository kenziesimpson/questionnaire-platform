...
- **L20** appends the `ExportDefaultDeclaration` selector to the ones `syntax()` inherits, and exempts `*.config.*` and the three framework `globalSetup`/`globalTeardown` files.
