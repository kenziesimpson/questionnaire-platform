...
REVOKE EXECUTE ON FUNCTION audit.record(text, uuid, uuid, int, text, jsonb, text) FROM qp_owner;
--> statement-breakpoint
REVOKE USAGE ON SCHEMA audit FROM qp_owner;
--> statement-breakpoint
RESET ROLE;
--> statement-breakpoint
DROP FUNCTION definition.promote_draft(uuid, jsonb, text, jsonb, text);
--> statement-breakpoint
CREATE FUNCTION definition.promote_draft(p_questionnaire_version_id uuid, p_snapshot jsonb)
...
END $$;
--> statement-breakpoint
REVOKE ALL ON FUNCTION definition.promote_draft(uuid, jsonb) FROM PUBLIC;
--> statement-breakpoint
