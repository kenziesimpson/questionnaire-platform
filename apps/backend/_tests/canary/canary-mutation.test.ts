...
describe("TELEMETRY CANARY mutation check: with the export-time scrub removed, a real-app flow fails the gate", () => {
  it("detects the sentinel in a span Fastify's instrumentation produced, not only in a planted one", async () => {
    const { CANARY_FLOWS } = await import("./flows.js");
    const { runOnCanaryApp } = await import("./harness.js");
    const { expectCleanRun } = await import("@qp/telemetry/canary");
    const flow = CANARY_FLOWS.find((candidate) => candidate.name.startsWith(REAL_APP_FLOW));
    expect(flow, "the real-app flow this check runs must still be registered").toBeDefined();
    if (flow === undefined) return;

    const run = await runOnCanaryApp(testDatabase, flow, { autoInstrumentation: true });
    ...
    expect(() => expectCleanRun(flow.name, run)).toThrow(/TELEMETRY CANARY FAILED/);
  });
});
