...
export async function anExecutionApp(testDatabase: TestDatabase, startAt: Date = NOW): Promise<ExecutionHarness> {
...
export function startSession(app: FastifyInstance, questionnaireId: string = INTAKE_QUESTIONNAIRE_ID) {
  return app.inject({ method: "POST", url: "/api/run/sessions", payload: { questionnaireId } });
}

export function getSession(app: FastifyInstance, sessionId: string) {
  return app.inject({ method: "GET", url: `/api/run/sessions/${sessionId}` });
}

export function submit(app: FastifyInstance, sessionId: string, answers: unknown) {
  return app.inject({ method: "POST", url: `/api/run/sessions/${sessionId}/submit`, payload: { answers } as object });
}
