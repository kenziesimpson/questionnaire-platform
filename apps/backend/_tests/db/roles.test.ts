...
describe("db/init/01-roles.sh", () => {
  it("creates qp_owner, qp_definition, qp_execution and qp_reporting as plain login roles and audit_owner without a login", async () => {
    ...
    const roles = await owner.query(
      `SELECT rolname, rolsuper, rolcanlogin, rolcreaterole, rolbypassrls
         FROM pg_roles WHERE rolname IN ('qp_owner', 'qp_definition', 'qp_execution', 'qp_reporting', 'audit_owner')
        ORDER BY rolname`,
    );
